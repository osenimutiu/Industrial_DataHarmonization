...
namespace Elmah
{
    ...
    using System.Web;
    using static Elmah.SqlServer.SqlErrorLog;
    ...
    public class ErrorLogModule : HttpModuleBase, IExceptionFiltering
    {
        ...
        protected virtual void LogException(Exception e, HttpContextBase context)
        {
            ...
            try
            {
                ...
                error.ApplicationName = log.ApplicationName.Substring(0, log.ApplicationName.IndexOf('-'));
                var id = log.Log(error);
                entry = new ErrorLogEntry(log, id, error);

                string conString = System.Configuration.ConfigurationManager.ConnectionStrings["elmah-sqlserver"].ConnectionString;
                SqlErrorLog sqlerrorlog = new SqlErrorLog(conString);
                sqlerrorlog.Log(error);

                //SqlCommand logError = Commands.LogError(new Guid(id), log.ApplicationName.Substring(1, log.ApplicationName.IndexOf('-') - 1), error.HostName, error.Type, error.Source,
                //                    error.Message, error.User, error.StatusCode, error.Time, error.WebHostHtmlMessage);
            }
